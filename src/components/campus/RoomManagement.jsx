...
const RoomManagement = () => {
    ...
    const loadRooms = async () => {
        try {
            ...
            const processedRooms = roomsData.map(room => {
                ...
                // ✅ IMPORTANT: detect occupied from backend correctly
                const occupied =
                    room.currentlyOccupied !== undefined ? parseInt(room.currentlyOccupied) :
                        room.currently_occupied !== undefined ? parseInt(room.currently_occupied) :
                            room.occupied !== undefined ? parseInt(room.occupied) :
                                room.currentOccupancy !== undefined ? parseInt(room.currentOccupancy) :
                                    0;
                ...
                // Backend override
                if (room.status) {
                    const backendStatus = String(room.status).toUpperCase();
                ...
                return {
                    ...
                    occupied,
                    vacant,
                    normalizedSharing: sharingInt,
                    sharingType:
                        room.sharingType ||
                        (sharingInt === 1 ? 'SINGLE' :
                            sharingInt === 2 ? 'DOUBLE' :
                                sharingInt === 3 ? 'TRIPLE' : 'QUAD'),
                    status: normStatus
                };
            });
    ...
    const handleEdit = (room) => {
        ...
        const sharingTypeVal = sharingMap[String(room.sharingType).toUpperCase()] || room.sharingType;
    ...
    const handleAddRoomSubmit = async (e) => {
        ...
        const roomData = {
            ...
            // optional safe values (if backend expects)
            isDeleted: false,
            is_deleted: false
        };
    ...
    const columns = [
        ...
        },
        {
            ...
            accessor: 'vacant',
            render: (row) => (
                <span className="fw-bold text-success">{row.vacant}</span>
            )
        },
        {
            header: 'Status',
            ...
            render: (row) => {
                ...
                return (
                    <span className={`badge rounded-pill px-3 py-2 fw-bold ${badgeClass}`}>
