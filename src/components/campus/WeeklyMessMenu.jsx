...
const WeeklyMessMenu = () => {
    ...
    const toBackendStatus = (frontendStatus) => {
        if (!frontendStatus) return 'NOT_SERVED';
        const statusMap = {
            'Not Served': 'NOT_SERVED',
            'Served': 'SERVED',
            'Completed': 'COMPLETED'
        };
        return statusMap[frontendStatus] || frontendStatus.toUpperCase().replace(/\s+/g, '_');
    };
    ...
    const loadMenu = async () => {
        try {
            ...
            // If backend returned data, map it
            if (data && data.length > 0) {
                days.forEach((d) => {
                    const dayMenu =
                        data.find((m) => m.day?.toUpperCase() === d.toUpperCase()) || null;
    ...
    if (!menuData[activeDay]) {
        return (
            <div className="p-5 text-center text-muted">Loading menu data...</div>
        );
    }
    ...
    const handleStatusChange = async (meal, newStatus) => {
        ...
        console.log(`[MessMenu] Updating ${meal} status to ${newStatus}`);
        ...
        try {
            if (currentParams.menuId) {
                await campusService.updateMenuFull(currentParams.menuId, updatePayload);
            } else {
                await campusService.createMenu(updatePayload);
                // Reload to capture the new ID created by backend
                await loadMenu();
            }
        } catch (e) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (currentParams.menuId) {
                await campusService.updateMenuFull(currentParams.menuId, payload);
            } else {
                await campusService.createMenu(payload);
            }
    ...
    const handleDelete = async () => {
        if (window.confirm("Are you sure you want to clear the menu for this day?")) {
            ...
            try {
                if (currentParams.menuId) {
                    await campusService.deleteMenu(currentParams.menuId);
                }
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'Not Served': default: return 'warning';
        }
    };
    ...
    const MealCard = ({ title, icon: Icon, data, colorClass }) => (
        <div
            className={`p-3 rounded-4 w-100 h-100 flex-grow-1 ${colorClass} bg-opacity-10 border border-${colorClass.replace(
                "bg-",
                ""
            )} border-opacity-20 shadow-sm transition-all hover-shadow-md d-flex flex-column overflow-hidden`}
            style={{ minHeight: "140px" }}
        >
            <div className="d-flex align-items-center justify-content-between mb-3">
                <div
                    className={`d-flex align-items-center gap-2 ${colorClass.replace(
                        "bg-",
                        "text-"
                    )} fw-bold text-uppercase tracking-wider smaller`}
                >
                    <Icon size={18} /> <span style={{ fontSize: "0.9rem" }}>{title}</span>
                </div>
                <span className={`badge bg-${getStatusColor(data.status)} bg-opacity-75 rounded-pill px-3 py-1 smaller shadow-sm d-flex align-items-center gap-1`}>
                    {data.status === 'Served' && <FaUtensils size={10} />}
                    {data.status === 'Not Served' && <FaClock size={10} />}
                    {data.status === 'Completed' && <FaCheck size={10} />}
                    {data.status}
                </span>
            </div>
            ...
            {data.items ? (
                <p
                    className="fw-500 text-dark mb-0 lh-lg text-break text-wrap flex-grow-1"
                    style={{
    ...
    return (
        <div className="container-fluid py-4 animate-in">
                ...
                <header className="mb-4 d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p className="text-muted small">
                            Manage menu items for each day.
                        </p>
                    </div>
                </header>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                        ...
                        {!isEditing ? (
                            <div className="d-flex gap-2">
                                <button
                                    className="btn btn-light text-danger fw-bold rounded-pill px-3"
                                ...
                                <button
                                    className="btn-premium btn-premium-primary rounded-pill px-4"
                            ...
                            ["Breakfast", "Lunch", "Dinner"].map((meal) => (
                                <div key={meal} className="col-md-4">
                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        {/* Status Selector UI */}
                                        <div className="d-flex bg-secondary bg-opacity-10 rounded-pill p-1 border shadow-sm align-items-center">
                                            {[
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        disabled={isEditing}
                                                        className={`btn btn-sm rounded-pill d-flex align-items-center gap-2 fw-bold transition-all border-0 ${isActive
                                                            ? `bg-white text-${statusOption.color} shadow-sm px-3 py-1`
                                                            : 'text-muted px-2 py-1 hover-bg-white-50'
                                                            }`}
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div
                                            className={`p-3 rounded-4 d-flex flex-column overflow-hidden ${meal === "Breakfast"
                                                ? "bg-warning"
                                                : meal === "Lunch"
                                                    ? "bg-danger"
                                                    : "bg-primary"
                                                } bg-opacity-10 border border-opacity-10`}
                                            style={{ height: "140px" }}
                                        >
                                            <p
                                                className="fw-500 mb-0 lh-lg text-break text-wrap"
                                                style={{
                                                    wordBreak: "break-word",
                                                    overflowWrap: "anywhere",
                                                    whiteSpace: "pre-line",
            ...
            <div className="pt-4 border-top border-light">
                ...
                <div
                    className="glass-card border-0 p-0 overflow-hidden shadow-2xl"
                    style={{ maxWidth: "1200px", margin: "0 auto" }}
                >
                    <div
                        className="p-4 bg-slate-900 text-white d-flex justify-content-between align-items-center"
                        style={{ background: "#0f172a" }}
                    >
                        <div>
                            <p className="text-success text-uppercase tracking-widest fw-bold smaller mb-1">
                                Live Menu Board
                            </p>
